...
func printUsage() {
	fmt.Println("http-v2 - Example x402 v2 payment client and server")
	fmt.Println()
	fmt.Println("This example demonstrates the x402 v2 protocol with CAIP-2 network identifiers.")
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  http-v2 server [flags]  - Run a test server with paywalled endpoints")
	fmt.Println("  http-v2 client [flags]  - Run client to access paywalled resources")
	fmt.Println()
	fmt.Println("Run 'http-v2 server --help' or 'http-v2 client --help' for more information.")
}
...
func runServer(args []string) {
	...
	if err != nil {
		// If not a known network, use the provided values directly
	...
	fmt.Printf("Starting HTTP x402 v2 server on port %s\n", *port)
	...
	// Note: v2 uses CAIP-2 network identifiers and atomic units directly
	...
	requirements := []v2.PaymentRequirements{requirement}

	// Create x402 v2 middleware
	...
	dataHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if paymentInfo := v2http.GetPaymentFromContext(r.Context()); paymentInfo != nil {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "HTTP x402 v2 Server\n\n")
		fmt.Fprintf(w, "Protocol: x402 version 2 with CAIP-2 network identifiers\n\n")
		fmt.Fprintf(w, "Endpoints:\n")
		fmt.Fprintf(w, "  GET /data    - Paywalled endpoint (requires x402 v2 payment)\n")
...
func runClient(args []string) {
	...
	network := fs.String("network", "eip155:84532", "Network to use (CAIP-2 format, e.g., eip155:8453, eip155:84532, solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp)")
