...
must_haves:
  truths:
    ...
    - "PaymentPayloadV2 has optional accepted field for nested requirements"
...
2. Define `PaymentPayloadV2` interface:
   ...
   - `payload: { signature: string; authorization: PaymentAuthorization; }`
   - `accepted?: PaymentRequirementsV2` (optional nested payment requirements for v2 format)
   - Add JSDoc: `/** Payment payload for x402 version 2. Uses CAIP-2 network identifiers. May include nested accepted requirements. */`
...
- PaymentPayloadV2 interface with literal `x402Version: 2` and nested `accepted` field
