...
import { z } from 'zod';
import { keccak256, toHex, parseAbiItem, parseAbi, parseEther, formatEther, formatUnits, formatGwei, parseGwei, decodeEventLog, encodeFunctionData, decodeFunctionResult, hashMessage, verifyMessage, recoverMessageAddress, hashTypedData, verifyTypedData, recoverTypedDataAddress, decodeFunctionData, getContract, parseUnits, createPublicClient, http, createWalletClient } from 'viem';
...
function registerContractTools(server) {
  ...
  server.tool(
    ...
    async ({ contractAddress, abi, functionName, args: args2 = [], network }) => {
      try {
...
function registerMulticallTools(server) {
  ...
  server.tool(
    ...
    async ({ functionSignature, args: args2 }) => {
      try {
        ...
        const processedArgs = args2.map((arg, i) => {
          const type = types[i];
...
var startServer = () => {
  try {
    ...
    logger_default.error("Failed to initialize server:", error);
    process.exit(1);
  }
};
...
var startSSEServer = async () => {
  try {
    ...
    app.use(cors());
    logger_default.info(`Starting sse server with log level: ${logger_default.getLevel()}`);
    const transports = {};
    app.get("/sse", async (_, res) => {
      const transport = new SSEServerTransport("/messages", res);
      transports[transport.sessionId] = transport;
      logger_default.info("New SSE connection established", {
        sessionId: transport.sessionId
      });
      res.on("close", () => {
        logger_default.info("SSE connection closed", { sessionId: transport.sessionId });
        delete transports[transport.sessionId];
      });
      try {
        await server.connect(transport);
      } catch (error) {
        logger_default.error("Error connecting transport", {
          sessionId: transport.sessionId,
          error
        });
      }
    ...
    app.post("/messages", async (req, res) => {
      ...
      if (transport) {
        logger_default.debug("Handling message", { sessionId, body: req.body });
        try {
          await transport.handlePostMessage(req, res, req.body);
        } catch (error) {
          logger_default.error("Error handling message", { sessionId, error });
          res.status(500).send("Internal server error");
        }
      } else {
        logger_default.warn("No transport found for session", { sessionId });
    ...
    logger_default.error("Error starting SperaxOS SSE Server:", error);
  }
};
var startStdioServer = async () => {
  try {
    ...
    logger_default.error("SperaxOS Server running on stdio mode");
    transport.onmessage = (message) => {
      logger_default.error("Received message:", message);
    };
    transport.onclose = () => {
      logger_default.error("Stdio server closed");
    };
    transport.onerror = (error) => {
      logger_default.error("Stdio server error:", error);
    };
    await server.connect(transport);
    return server;
  } catch (error) {
    logger_default.error("Error starting SperaxOS Stdio server:", error);
  }
};
...
async function main() {
  ...
  if (!server) {
    logger_default.error("Failed to start server");
    process.exit(1);
  }
